...
本轮累计岭拟合尝试 5399 / 8000（包含合成测试及失败尝试）；全部通过 Slurm，申请时限累计上界 3.250 CPU core·h，0 GPU。逐次作业见 jobs.json；sacct 记账查询状态为 UNAVAILABLE，查询输出/错误留在 private。若记账服务不可用，不能将申请上界当成实际使用量。
