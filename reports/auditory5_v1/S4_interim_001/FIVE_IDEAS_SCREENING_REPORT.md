...
执行状态：**IN_PROGRESS**；表征状态：**IN_PROGRESS**（79/90）。
...
| 路线 | 主终点 | verdict | 控制 | 科学完成 |
|---|---|---|---|---|
| A | post_T | NEED_CONTROLS | PENDING | False |
| B | calibrated_main_gain | NEED_CONTROLS | PENDING | False |
| C | T_C_mlp32_calibrated | IMPLEMENTATION_FAIL | FAIL | False |
| D | D2_minus_D3 | NEED_CONTROLS | PENDING | False |
| E | E1_transfer_gap_not_estimated | IMPLEMENTATION_FAIL | FAIL | False |
...
待完成：D_SIM_core_001:missing；D_controls_SIM_001:missing；R_SIM:primary_estimate
...
精确输入与代码 SHA256 见 source_hashes.csv；路径与异常详情仅写 private 审计。补充控制估计见 control_metrics.csv；所有任务/分支/窗口刺激读出见 stimulus_decoding.csv。
